...
/**
 * =========================================
 * 🔍 Dernier code caisse par agence
 *
 * ex:
 * agence 100000
 * -> 100003
 * =========================================
 */
export async function getLastCaisseCodeByAgence(
  agence_id: string
) {
  const result = await query(
    `
    ...
    ORDER BY CAST(code_caisse AS BIGINT) DESC
    LIMIT 1
    `,
    [agence_id]
  );
 ...
 * =========================================
 */
export async function createCaisse(
  data: any
) {
  const result = await query(
    `
...
export async function getAllCaisses(
  limit: number,
  offset: number
) {
...
export async function getCaissesByAgent(
  agent_id: string
) {
  return await query(
    `SELECT * FROM caisse
...
export async function getCaisseById(
  id: string
) {
  const result = await query(
    `SELECT 
...
export async function getCaissesByAgence(
  agence_id: string
) {
  return await query(
    `SELECT 
...
export async function updateCaisse(
  id: string,
  data: any
) {
  const fields: string[] = [];
  ...
  if (!result[0]) {
    throw new Error(
      'Caisse introuvable ou désactivée'
    );
  }
...
export async function updateCaisseState(
  id: string,
  state: string
) {
  const result = await query(
...
export async function softDeleteCaisse(
  id: string
) {
  const result = await query(
    `UPDATE caisse
